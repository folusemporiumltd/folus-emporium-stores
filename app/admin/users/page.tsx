...
export default async function AdminUsersPage({searchParams}:{searchParams:Promise<{updated?:string,error?:string}>}){
  ...
  return <main><div className="topbar"><div className="container"><span>Folus Emporium Stores Administration</span><span>Roles & access</span></div></div><header className="nav"><div className="container nav-inner"><Link className="brand" href="/admin/dashboard"><img src="/folus-emporium-circular-logo.png" alt="Folus Emporium Stores logo"/><span>FOLUS<br/>EMPORIUM STORES<small>Admin Role Manager</small></span></Link><nav className="navlinks"><Link href="/admin/dashboard">Dashboard</Link><Link href="/admin">Store Manager</Link><Link href="/admin/coupons">Coupons</Link><Link href="/">Storefront</Link></nav></div></header><section className="section"><div className="container" style={{maxWidth:1050}}><AdminBreadcrumbs items={[{label:'Admin',href:'/admin/dashboard'},{label:'Roles & Access'}]}/><div className="eyebrow">Access control</div><h1>Manage registered accounts</h1><p className="muted">View every registered Folus Emporium Stores account and assign either Administrator or Customer access. Your own signed-in administrator account is protected from self-demotion.</p>
