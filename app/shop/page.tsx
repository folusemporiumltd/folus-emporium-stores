...
export async function generateMetadata({searchParams}:{searchParams:Promise<Params>}) {
 ...
 return { title: isNew ? 'New Arrivals | Folus Emporium Stores' : 'Shop | Folus Emporium Stores', description: isNew ? 'Discover the latest additions to the Folus Emporium Stores collection. Browse products added in the last 30 days.' : 'Shop the Folus Emporium Stores collection by category, size and price.' }
}
...
export default async function ShopPage({searchParams}:{searchParams:Promise<Params>}) {
 ...
 const defaultVariant=new Map<string,Variant>(); for(const [id,list] of byProduct){defaultVariant.set(id,list.find(v=>v.size_grams===products.find(p=>p.id===id)?.default_size_grams)??list[0])}
