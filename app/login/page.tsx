      ...
      <section className="section">
        <div className="container" style={{ maxWidth: 680 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 28 }}>
            <img src="/folus-emporium-circular-logo.png" alt="Folus Emporium Stores logo" style={{ width: 58, height: 58, objectFit: 'contain' }} />
            <div><strong style={{ color: 'var(--burgundy)', fontSize: 18 }}>FOLUS EMPORIUM</strong><div className="muted">Curating Excellence for Life’s Finest Moments.</div></div>
          </div>
          ...
          <p>
            {mode === 'signup'
              ? 'Create your Folus Emporium Stores account. We will send a confirmation email before you can sign in and continue to checkout.'
              : isCheckout
                ? 'Sign in to continue from your cart to the secure Paystack payment step.'
                : 'Sign in to access your Folus Emporium Stores account.'}
          </p>
          ...
          {mode === 'signup' ? (
            <form>
              ...
              <label htmlFor="newsletter_consent" style={{display:'flex',alignItems:'flex-start',gap:10,fontWeight:500,margin:'18px 0'}}>
                ...
                <span>I would like to receive Folus Emporium Stores offers, new product updates and newsletters by email. I can unsubscribe at any time.</span>
              </label>
          ...
          <p style={{ marginTop: 20 }}>
            {mode === 'signup' ? (
              ...
              <>New to Folus Emporium Stores? <Link className="auth-switch-btn" href={`/login?next=${encodeURIComponent(next)}&mode=signup`}>Create an account</Link></>
            )}
          </p>
        </div>
      </section>
