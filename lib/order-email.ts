...
function brevoSender(){
  ...
  return match?{name:match[1]||'Folus Emporium Stores',email:match[2]}:{name:'Folus Emporium Stores',email:value}
}
function titleFor(eventKey:string){
  if(eventKey==='purchase_thank_you') return 'Thank you for patronizing Folus Emporium Stores'
  if(eventKey==='payment_confirmed') return 'Payment confirmed — Folus Emporium Stores'
  if(eventKey==='pay_after_delivery_confirmed') return 'Order confirmed — Pay After Delivery'
  if(eventKey.startsWith('status_')) return `Order ${eventKey.slice(7).replaceAll('_',' ')} — Folus Emporium Stores`
  return 'Order update — Folus Emporium Stores'
}
function messageFor(eventKey:string){
  const s=eventKey.replace(/^status_/,'')
  const map:Record<string,string>={processing:'Your order is now being prepared.',shipped:'Your order has been shipped and is on the way.',delivered:'Your order has been delivered. Thank you for shopping with Folus Emporium Stores.',cancelled:'Your order has been cancelled.',pending:'Your order is pending review.'}
  if(eventKey==='payment_confirmed') return 'We have received your payment successfully and your order is confirmed.'
  if(eventKey==='purchase_thank_you') return 'Thank you for choosing Folus Emporium Stores. We would love to hear what you think about the products in your order.'
