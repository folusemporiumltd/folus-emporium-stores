...
export default async function StoreDashboard({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <main>
    ...
    <section className="section"><div className="container" style={{maxWidth:1050}}>
      ...
      <section className="admin-dashboard-panel" style={{marginTop:28}}><h2>Manage this store</h2><p className="muted">Manage Folus Emporium Stores records separately from the main website.</p><div style={{display:'flex',gap:12,flexWrap:'wrap'}}><Link className="btn btn-primary" href="/admin/products">Store products</Link><Link className="btn btn-outline" href="/admin/orders">Store orders</Link><Link className="btn btn-outline" href="/admin/inventory">Inventory</Link><Link className="btn btn-outline" href="/admin/coupons">Store coupons</Link><Link className="btn btn-outline" href="/admin/reviews">Store reviews</Link><Link className="btn btn-outline" href="/admin/reports">Reports</Link><Link className="btn btn-outline" href="/admin/customers">Store customers</Link><Link className="btn btn-outline" href="/admin/storefront">Storefront</Link><Link className="btn btn-outline" href="/admin/company">Store content</Link></div></section>
    </div></section>
  </main>
}
