...
export const metadata: Metadata={title:'Careers | Folus Emporium Stores',description:'Explore career opportunities and learn what it means to work with Folus Emporium Stores.'}
...
const fallback={eyebrow:'Careers',title:'Grow with a business that is still being built.',intro:'Folus Emporium Stores is developing its food, ecommerce, operations and customer-service capabilities. We value practical thinkers who are willing to learn, take ownership and help improve the way work gets done.',content:'We value ownership, customer focus, continuous improvement, professionalism and respect. Openings are advertised when positions become available.',secondary_title:'Current openings',secondary_content:'No publicly advertised vacancy at the moment. If you would like to be considered for future opportunities, send a concise CV and a short introduction explaining the area in which you can contribute.'}
...
export default async function CareersPage(){
 ...
 return <main>
  <header className="nav"><div className="container nav-inner"><Link className="brand" href="/"><img src="/folus-emporium-circular-logo.png" alt="Folus Emporium Stores logo"/><span>FOLUS<br/>EMPORIUM STORES<small>Curating Excellence for Life’s Finest Moments.</small></span></Link><nav className="navlinks"><Link href="/">Home</Link><Link href="/shop">Shop</Link><Link href="/about">About Us</Link><Link href="/blog">Blog</Link></nav></div></header>
  <section className="content-hero"><div className="container content-hero-grid"><div><div className="eyebrow">{c.eyebrow||'Careers'}</div><h1>{c.title}</h1>{paras(c.intro)}<div className="hero-actions"><a className="btn btn-primary" href="mailto:info@folusemporium.com?subject=Career%20Interest%20-%20Folus%20Emporium">Send your CV</a><Link className="btn btn-outline" href="/about">Learn about us</Link></div></div><div className="content-brand-card"><span className="eyebrow">Working at Folus Emporium Stores</span><strong>Small team. Real responsibility. Visible impact.</strong><p>As we grow, each team member has an opportunity to contribute ideas, strengthen systems and improve customer outcomes.</p></div></div></section>
  ...
  <section className="cta"><div className="container"><div className="cta-box"><div><div className="eyebrow">Know our business first</div><h2>Read the story behind Folus Emporium Stores.</h2></div><Link className="btn btn-primary" href="/about#story">Our Story</Link></div></div></section>
 </main>
}
