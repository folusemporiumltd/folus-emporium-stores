...
export async function POST(request: Request) {
  try {
    ...
    const payload: Record<string, unknown> = {
      email,
      listIds: Number.isFinite(listId) && listId > 0 ? [listId] : [],
      updateEnabled: true,
    }
    ...
    if (shouldSendWelcome && Number(process.env.BREVO_NEWSLETTER_WELCOME_TEMPLATE_ID) > 0) {
      const templateId = Number(process.env.BREVO_NEWSLETTER_WELCOME_TEMPLATE_ID || '0')
      const welcomeResponse = await fetch('https://api.brevo.com/v3/smtp/email', {
        method: 'POST',
        headers: { accept: 'application/json', 'content-type': 'application/json', 'api-key': apiKey },
        body: JSON.stringify({ to: [{ email }], templateId: Number.isFinite(templateId) && templateId > 0 ? templateId : 0 }),
        cache: 'no-store',
      })
