...
async function sendCustomerWelcomeEmail(email: string) {
  ...
  const configuredTemplateId = Number(process.env.BREVO_CUSTOMER_WELCOME_TEMPLATE_ID || '0')
  if (!Number.isInteger(configuredTemplateId) || configuredTemplateId <= 0) return
