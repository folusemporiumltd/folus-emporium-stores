...
export async function POST(request:Request){
  ...
  const prompt=`Prepare one professional Folus Emporium Stores business email from the instruction below. Use only facts supported by the supplied Gmail context and instruction. If a recipient can be identified from the instruction or a clearly matching recent message, use that email address. Otherwise leave the recipient as an empty string. Do not invent order numbers, payments, promises, dates or customer facts. Tone: warm, concise, professional.\n\nSENDER ACCOUNT: ${accountEmail}\n\nINSTRUCTION:\n${instruction}\n\nRECENT GMAIL CONTEXT:\n${JSON.stringify(account.recent_messages||[])}`
