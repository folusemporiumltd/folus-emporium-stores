...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (error || !orderId) return NextResponse.json({ error: error?.message || 'Payment succeeded, but we could not confirm the order. Please contact Folus Emporium Stores with your payment reference.' }, { status: 500 })
