...
export default function FAQPage() {
  return <main className="support-page" id="faq"><header className="nav"><div className="container nav-inner"><Link className="brand" href="/"><img src="/folus-emporium-circular-logo.png" alt="Folus Emporium Stores logo"/><span>FOLUS<br/>EMPORIUM STORES<small>Curating Excellence for Life’s Finest Moments.</small></span></Link><nav className="navlinks"><Link href="/">Home</Link><Link href="/shop">Shop</Link><Link href="/contact">Contact Us</Link></nav></div></header><section className="section"><div className="container"><div className="eyebrow">Customer Service</div><h1>Frequently asked questions</h1><p>Helpful answers for ordering from Folus Emporium Stores. Need more help? <Link href="/contact">Contact us</Link>.</p><div className="faq-list">{questions.map(([question, answer]) => <details key={question}><summary>{question}</summary><p>{answer}</p></details>)}</div></div></section></main>
}
