...
async function subscribe(formData:FormData){
  ...
  redirect('/newsletter?message='+encodeURIComponent('Thank you. You are now subscribed to Folus Emporium Stores updates.'))
}
...
export default async function NewsletterPage({searchParams}:{searchParams:Promise<{unsubscribe?:string,message?:string,error?:string}>}){
  ...
  return <main><header className="nav"><div className="container nav-inner"><Link className="brand" href="/"><img src="/folus-emporium-circular-logo.png" alt="Folus Emporium Stores logo"/><span>FOLUS<br/>EMPORIUM STORES<small>Curating Excellence for Life’s Finest Moments.</small></span></Link><nav className="navlinks"><Link href="/">Home</Link><Link href="/shop">Shop</Link><Link href="/login">My Account</Link></nav></div></header><section className="section"><div className="container" style={{maxWidth:680}}><div className="eyebrow">Stay connected</div><h1>Folus Emporium Stores Newsletter</h1><p className="muted">Receive new product announcements, useful food and lifestyle updates, and selected Folus Emporium Stores offers.</p>
  ...
  {token?<div className="cart-summary" style={{position:'static',marginTop:24}}><h2>Unsubscribe</h2><p>Stop receiving Folus Emporium Stores promotional newsletters at this email address?</p><form action={unsubscribe}><input type="hidden" name="token" value={token}/><button className="btn btn-primary" type="submit">Confirm unsubscribe</button></form></div>:<div className="cart-summary" style={{position:'static',marginTop:24}}><h2>Join our mailing list</h2><form action={subscribe} style={{display:'grid',gap:14}}><label>Full name <span className="muted">(optional)</span><input name="full_name" autoComplete="name"/></label><label>Email address<input name="email" type="email" autoComplete="email" required/></label><p className="muted" style={{margin:0,fontSize:13}}>By subscribing, you agree to receive Folus Emporium Stores marketing emails. You can unsubscribe at any time.</p><div><button className="btn btn-primary" type="submit">Subscribe</button></div></form></div>}
 </div></section></main>
}
