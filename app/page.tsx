...
const categories = [
  [
    ...
    "Kitchen Appliances",
    "kitchen-appliances",
    "Practical appliances for preparing and enjoying meals."
  ],
  [
    "🎁",
    "Souvenirs & Gifts",
    "souvenirs-gifts",
    "Thoughtful gifts and keepsakes for special occasions."
  ],
  [
    "🥤",
    "Food & Beverages",
    "food-beverages",
    "Food, drinks and pantry favourites for everyday living."
  ],
  [
    "🧼",
    "Hygiene & Personal Care",
    "hygiene-personal-care",
    "Everyday essentials for personal and household care."
  ],
  [
    "✨",
    "Health & Beauty",
    "health-beauty",
    "Beauty and self-care products for your daily routine."
  ],
  [
    "👗",
    "Fashion, Accessories & Apparel",
    "fashion-accessories-apparel",
    "Clothing and accessories for your personal style."
  ],
  [
    "🏠",
    "Home Appliances",
    "home-appliances",
    "Useful appliances for a comfortable, well-equipped home."
  ]
]

const promises = [
  { title: 'CHOICE', description: 'Useful products for your home, style and celebrations.', icon: 'leaf' },
  { title: 'QUALITY', description: 'Products selected with attention to quality.', icon: 'shield' },
  { title: 'CONVENIENCE', description: 'Shop your everyday essentials in one place.', icon: 'sparkles' },
...
export default async function Home({ searchParams }: { searchParams: Promise<{ code?: string }> }) {
  ...
  const [{ data: products }, { data: storefrontContent }, { data: headerCategories }] = await Promise.all([
    supabase.from('products').select('id,name,slug,description,price,image_url,featured,stock_quantity,default_size_grams').eq('site_key', SITE_KEY).eq('is_active', true).order('featured', { ascending: false }).order('created_at', { ascending: false }),
  ...
  for (const variant of variants ?? []) { const existing = defaultVariantByProduct.get(variant.product_id); if (!existing || variant.size_grams === products?.find(p => p.id === variant.product_id)?.default_size_grams) defaultVariantByProduct.set(variant.product_id, variant) }
  return <main>
    <StorefrontHeader config={storefrontConfig} categories={headerCategories ?? []}/>
    <section className="hero"><div className="container hero-grid"><div><div className="eyebrow">Welcome to Folus Emporium Stores</div><h1>Everyday essentials.<br/><em>Exceptional choices.</em></h1><p>Shop appliances, gifts, food, personal care, beauty and fashion—all in one place, selected to make everyday living simpler.</p><div className="hero-actions"><Link className="btn btn-primary" href="/shop">Explore the Collection</Link><a className="btn btn-outline" href="https://wa.me/2349168157255">Chat on WhatsApp</a></div></div><div className="hero-card"><div className="quote"><div className="eyebrow">Our promise</div><strong>Thoughtful choices. Dependable quality. Personal service.</strong><p>From home essentials to thoughtful gifts, we select products with care and help you find the right choice for your needs.</p></div></div></div></section>
    <section className="section home-featured-section"><div className="container"><div className="section-head"><div><div className="eyebrow">Featured</div><h2>Our product collection</h2></div><Link className="btn btn-outline" href="/shop">View all products</Link></div>{products?.length ? <FeaturedProductCarousel>{products.map((p) => { const variant = defaultVariantByProduct.get(p.id); return <article className="product-card" key={p.id}><Link href={`/shop/${p.slug}`} aria-label={`View ${p.name}`}><div className="product-image">{p.image_url ? <img src={p.image_url} alt={p.name} style={{width:'100%',height:'100%',objectFit:'contain'}}/> : <span>{p.name}</span>}</div></Link><div className="product-body"><span className="status">{p.featured ? 'Featured' : 'Folus collection'} · {variant?.size_label ?? 'Standard'}</span><h3><Link href={`/shop/${p.slug}`}>{p.name}</Link></h3><p>{p.description}</p><strong>{variant && Number(variant.price) > 0 ? `₦${Number(variant.price).toLocaleString('en-NG')}` : Number(p.price) > 0 ? `₦${Number(p.price).toLocaleString('en-NG')}` : 'Price to be updated'}</strong><ProductCardActions product={p} variant={variant}/></div></article> })}</FeaturedProductCarousel> : <div className="empty"><h3>Our collection is being prepared.</h3><p>Product catalogue setup is underway. Check the Shop page as we continue adding the collection.</p><Link className="btn btn-primary" href="/shop">Visit Shop</Link></div>}</div></section>
    <div className="container home-section-divider" aria-hidden="true"/>
    <section id="categories" className="section home-category-section"><div className="container"><div className="section-head"><div><div className="eyebrow">Explore</div><h2>Shop by category</h2></div><p>Explore seven collections for your home, personal style, celebrations and everyday needs.</p></div><div className="categories">{categories.map(([icon,title,slug,desc]) => <Link className="category" href={`/shop?category=${slug}`} key={title}><div className="icon">{icon}</div><h3>{title}</h3><p>{desc}</p></Link>)}</div></div></section>
    <section className="section promise"><div className="container"><div className="section-head"><div><div className="eyebrow">Why Folus Emporium Stores</div><h2 style={{color:'#fff'}}>Curated with intention.</h2></div></div><div className="promise-grid">{promises.map((promise) => <div className="promise-card" key={promise.title}><div className="promise-icon"><PromiseIcon type={promise.icon} /></div><b>{promise.title}</b><p>{promise.description}</p></div>)}</div></div></section>
    <section className="cta"><div className="container"><div className="cta-box"><div><div className="eyebrow">For homes & businesses</div><h2>Find something for every part of your day.</h2></div><Link className="btn btn-primary" href="/shop">Start Shopping</Link></div></div></section>
    ...
    </div><div className="container footer-social-row"><div><b>Follow Folus Emporium Stores</b><div className="footer-socials">{social.facebook?<a href={social.facebook} target="_blank" rel="noreferrer" aria-label="Facebook">f</a>:null}{social.instagram?<a href={social.instagram} target="_blank" rel="noreferrer" aria-label="Instagram">◎</a>:null}{social.tiktok?<a href={social.tiktok} target="_blank" rel="noreferrer" aria-label="TikTok">♪</a>:null}{social.youtube?<a href={social.youtube} target="_blank" rel="noreferrer" aria-label="YouTube">▶</a>:null}{social.linkedin?<a href={social.linkedin} target="_blank" rel="noreferrer" aria-label="LinkedIn">in</a>:null}</div></div><div className="footer-payment-logos" role="group" aria-label="Accepted payment methods" style={{display:'flex',alignItems:'center',flexWrap:'nowrap',gap:14,padding:'10px 12px',background:'#fffdf9',border:'1px solid #e2ad46',borderRadius:8,maxWidth:'100%',boxSizing:'border-box'}}><img src="/Paystack.png" alt="Paystack" width={116} height={28} style={{display:'block',width:116,height:28,objectFit:'cover',flexShrink:1,minWidth:0,mixBlendMode:'multiply'}}/><img src="/visa-and-mastercard-logo-26.png" alt="Visa and Mastercard" width={132} height={34} style={{display:'block',width:132,height:34,objectFit:'contain',flexShrink:1,minWidth:0,mixBlendMode:'multiply'}}/></div></div><div className="container copyright footer-bottom"><img src="/folus-emporium-circular-logo.png" alt="Folus Emporium Stores"/><span>© 2026 Folus Emporium Stores. All Rights Reserved. · Curating Excellence for Life’s Finest Moments.</span></div></footer>
  </main>
}
